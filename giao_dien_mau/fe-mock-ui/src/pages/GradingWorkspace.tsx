...
import { gradingData, badgesData, myClassesData, assignmentListData } from '../mockData';
...
export default function GradingWorkspace() {
  ...
  const { students, submissions } = gradingData;
  ...
  if (!selectedAssignmentId) {
    const filteredAssignments = selectedClassId 
      ? assignmentListData.filter(a => a.className === myClassesData.find(c => c.id === selectedClassId)?.name)
      : [];
              ...
              {myClassesData.map(c => (
                ...
                >
                  <div>
                    ...
                    <p className={cn("text-sm mt-1", selectedClassId === c.id ? "text-blue-100" : "text-slate-500")}>{c.studentsCount} học sinh</p>
                  </div>
          ...
          <div className="w-2/3 bg-white flex flex-col">
            <div className="p-5 border-b border-slate-100">
              <h3 className="font-bold text-slate-800">
                {selectedClassId ? `Bài tập cần chấm - ${myClassesData.find(c => c.id === selectedClassId)?.name}` : "Bài tập cần chấm"}
              </h3>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in">
                  {filteredAssignments.map(a => (
                    ...
                    >
                      ...
                      <h4 className="font-bold text-slate-900 group-hover:text-blue-700 transition leading-relaxed mb-3">{a.title}</h4>
                      
                      <div className="mt-auto space-y-2 w-full">
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-slate-500">Đã nộp:</span>
                          <span className="font-bold text-blue-600">{a.submitted}/{a.total}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-slate-500">Hạn nộp:</span>
                          <span className="font-medium text-slate-700">{a.deadlineText.split(' ')[0]}</span>
                        </div>
                      </div>
                    </button>
                  ))}
  ...
  const activeAssignment = assignmentListData.find(a => a.id === selectedAssignmentId);
  const activeStudentInfo = students.find(s => s.id === selectedStudent);
  const currentSubmission = (submissions as any)[selectedStudent];
  const isAutoGraded = currentSubmission?.type === 'TRAC_NGHIEM';
  ...
  const handleGenerateAIComment = () => {
    ...
    setTimeout(() => {
      setComment('Bài làm rất tốt, em đã hiểu rõ bản chất của đồ thị Parabol. Tuy nhiên cần chú ý trình bày cẩn thận hơn.');
  ...
  const handleAwardBadgeSubmit = (e: React.FormEvent) => {
    ...
    toast.success(`Đã tặng huy hiệu cho ${activeStudentInfo?.name} thành công!`);
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedStudent === student.id 
                    ? "bg-slate-900 shadow-sm" // Dùng màu trung tính đậm để làm nổi bật thay vì xanh thương hiệu
                    : "hover:bg-slate-50 text-slate-600"
                )}
              >
                <div>
                  <p className={cn("font-medium text-sm", selectedStudent === student.id ? "text-white" : "text-slate-900")}>
                    {student.name}
                  </p>
                  <p className={cn("text-xs mt-0.5", selectedStudent === student.id ? "text-slate-400" : "text-slate-500")}>
                    {student.time}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between pb-6 border-b border-slate-200">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-white border border-slate-200 text-slate-700 rounded-full flex items-center justify-center font-bold text-xl shadow-sm">
                    {activeStudentInfo?.name.charAt(0)}
                  </div>
                  <div>
                    <h3 className="font-bold text-xl text-slate-900">{activeStudentInfo?.name}</h3>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-8 border-b border-slate-100">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-2">Động viên học sinh <strong className="text-slate-700">{activeStudentInfo?.name}</strong> bằng một danh hiệu xứng đáng.</p>
              </div>
                ...
                {badgesData.map((badge) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedBadge === badge.id ? "border-orange-500 ring-4 ring-orange-50" : "border-slate-200 hover:border-slate-300"
                    )}
                  >
                    <div className={cn("w-14 h-14 shrink-0 rounded-full flex items-center justify-center text-3xl", badge.color)}>
                      {badge.icon}
                    </div>
                    <div>
