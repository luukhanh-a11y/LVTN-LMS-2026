...
CREATE TABLE dang_bai (
    ...
    du_lieu_game        JSON            NULL
                        COMMENT 'Lưu cấu hình game, câu hỏi, nội dung tĩnh...',
    dap_an_chuan        JSON            NULL
                        COMMENT 'Lưu đáp án chuẩn để chấm điểm tự động...',
...
COMMENT='Dạng bài kiêm nội dung học (đa hình qua du_lieu_game, dap_an_chuan JSON). GV có thể thêm bài bổ sung.';
...
COMMENT='Thông báo (Nội dung cốt lõi)';


-- ── 22. HỘP THƯ THÔNG BÁO (INBOX) ────────────────────────
CREATE TABLE hop_thu_thong_bao (
    hop_thu_id      BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    nguoi_dung_id   BIGINT UNSIGNED NOT NULL,
    thong_bao_id    BIGINT UNSIGNED NOT NULL,
    da_doc          TINYINT(1)      NOT NULL DEFAULT 0,
    thoi_diem_doc   DATETIME        NULL,
    UNIQUE KEY uk_hop_thu_thong_bao (nguoi_dung_id, thong_bao_id),
    CONSTRAINT fk_ht_nd
        FOREIGN KEY (nguoi_dung_id) REFERENCES nguoi_dung (nguoi_dung_id)
        ON DELETE CASCADE,
    CONSTRAINT fk_ht_tb
...
COMMENT='Hộp thư cá nhân cho từng người dùng';
...
CREATE TABLE tien_do_hoc_sinh (
    ...
    -- UNIQUE theo (HS, bài học, học kỳ) — không ghi đè kỳ cũ
    UNIQUE KEY uk_hs_baihoc_ky (hoc_sinh_id, bai_hoc_id, hoc_ky_id),
...
COMMENT='Tiến độ bài học theo năm học + học kỳ — tránh ghi đè khi học lại; đầu vào AI';
...
-- ── 24. LỊCH SỬ TỰ HỌC ─────────────────────────────────
CREATE TABLE lich_su_tu_hoc (
    tu_hoc_id           BIGINT UNSIGNED  NOT NULL AUTO_INCREMENT,
    hoc_sinh_id         BIGINT UNSIGNED  NOT NULL,
    dang_bai_id         INT UNSIGNED     NOT NULL,
    diem_so             DECIMAL(5,2)     NULL,
    chi_tiet_bai_lam    JSON             NULL,
    thoi_gian_nop       DATETIME         NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (tu_hoc_id),
    CONSTRAINT fk_lsth_hoc_sinh
        FOREIGN KEY (hoc_sinh_id) REFERENCES ho_so_hoc_sinh (hoc_sinh_id)
        ON DELETE CASCADE,
    CONSTRAINT fk_lsth_dang_bai
        FOREIGN KEY (dang_bai_id) REFERENCES dang_bai (dang_bai_id)
        ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Lịch sử nộp bài tự luyện tập của học sinh';
...
INSERT INTO dang_bai
    (dang_bai_id, bai_hoc_id, book_index_id_ngoai, mon_hoc_id, ten_dang_bai, slug, so_trang, so_thu_tu, du_lieu_game, dap_an_chuan)
VALUES
    -- Bài 1: Các số 0,1,2,3,4,5 (muc_luc_id=11)
    (1,  11, 16145, 1, 'Khám phá',  'kham-pha',  8,  0, '{"loai":"kham_pha","mo_ta":"Quan sát và nhận biết các số 0-5"}', NULL),
    (2,  11, 16147, 1, 'Hoạt động', 'hoat-dong', 9,  1, '{"loai":"hoat_dong","mo_ta":"Thực hành viết và đếm số 0-5"}', NULL),
    (3,  11, 16148, 1, 'Luyện tập', 'luyen-tap', 10, 2, '{"loai":"luyen_tap","mo_ta":"Bài tập củng cố số 0-5 trang 10"}', NULL),
    (4,  11, 16149, 1, 'Luyện tập', 'luyen-tap', 12, 3, '{"loai":"luyen_tap","mo_ta":"Bài tập củng cố số 0-5 trang 12"}', NULL),
    -- Bài 2: Các số 6,7,8,9,10 (muc_luc_id=12)
    (5,  12, 16150, 1, 'Khám phá',  'kham-pha',  14, 1, '{"loai":"kham_pha","mo_ta":"Quan sát và nhận biết các số 6-10"}', NULL),
    (6,  12, 16151, 1, 'Hoạt động', 'hoat-dong', 15, 1, '{"loai":"hoat_dong","mo_ta":"Thực hành viết và đếm số 6-10"}', NULL),
    (7,  12, 16152, 1, 'Luyện tập', 'luyen-tap', 16, 2, '{"loai":"luyen_tap","mo_ta":"Bài tập củng cố số 6-10 trang 16"}', NULL),
    (8,  12, 16153, 1, 'Luyện tập', 'luyen-tap', 18, 3, '{"loai":"luyen_tap","mo_ta":"Bài tập củng cố số 6-10 trang 18"}', NULL),
    -- Bài 4: So sánh số — nhiều Khám phá/Hoạt động xen kẽ (muc_luc_id=14)
    (9,  14, 16157, 1, 'Khám phá',  'kham-pha',  24, 1, '{"loai":"kham_pha","mo_ta":"So sánh số — tiết 1 trang 24"}', NULL),
    (10, 14, 16158, 1, 'Hoạt động', 'hoat-dong', 24, 2, '{"loai":"hoat_dong","mo_ta":"Thực hành so sánh — tiết 1"}', NULL),
    (11, 14, 16159, 1, 'Khám phá',  'kham-pha',  26, 3, '{"loai":"kham_pha","mo_ta":"So sánh số — tiết 2 trang 26"}', NULL),
    (12, 14, 16160, 1, 'Hoạt động', 'hoat-dong', 26, 4, '{"loai":"hoat_dong","mo_ta":"Thực hành so sánh — tiết 2"}', NULL),
    (13, 14, 16161, 1, 'Khám phá',  'kham-pha',  28, 5, '{"loai":"kham_pha","mo_ta":"So sánh số — tiết 3 trang 28"}', NULL),
    (14, 14, 16162, 1, 'Hoạt động', 'hoat-dong', 28, 6, '{"loai":"hoat_dong","mo_ta":"Thực hành so sánh — tiết 3"}', NULL),
    (15, 14, 16163, 1, 'Luyện tập', 'luyen-tap', 30, 7, '{"loai":"luyen_tap","mo_ta":"Luyện tập tổng hợp so sánh số"}', NULL),
    -- Bài 10: Phép cộng (muc_luc_id=30)
    (16, 30, 17854, 1, 'Khám phá',  'kham-pha',  56, 0, '{"loai":"kham_pha","mo_ta":"Phép cộng — tiết 1 trang 56"}', NULL),
    (17, 30, 17855, 1, 'Hoạt động', 'hoat-dong', 56, 1, '{"loai":"hoat_dong","mo_ta":"Thực hành cộng — tiết 1"}', NULL),
    (18, 30, 17856, 1, 'Luyện tập', 'luyen-tap', 58, 2, '{"loai":"luyen_tap","mo_ta":"Luyện tập cộng trang 58"}', NULL),
    -- Bài 11: Phép trừ (muc_luc_id=31)
    (19, 31, 17863, 1, 'Khám phá',  'kham-pha',  68, 0, '{"loai":"kham_pha","mo_ta":"Phép trừ — tiết 1 trang 68"}', NULL),
    (20, 31, 17864, 1, 'Hoạt động', 'hoat-dong', 69, 1, '{"loai":"hoat_dong","mo_ta":"Thực hành trừ — tiết 1"}', NULL),
    (21, 31, 17867, 1, 'Luyện tập', 'luyen-tap', 72, 4, '{"loai":"luyen_tap","mo_ta":"Luyện tập trừ trang 72"}', NULL);
