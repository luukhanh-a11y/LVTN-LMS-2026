...
import { dashboardData, myClassesData } from '../mockData';

export default function Dashboard() {
  const { heroMetrics, studentAlerts } = dashboardData;

  // Lấy tối đa 3 lớp để hiển thị nhanh trên Dashboard
  const recentClasses = myClassesData.slice(0, 3);
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Metric 1 */}
          <Link to="/classes" className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 hover:shadow-md hover:border-blue-200 transition group block">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900">{heroMetrics.totalClasses}</p>
            </div>
          </Link>

          {/* Metric 2: Điểm nhấn màu Cam/Đỏ */}
          <Link to="/grading" className="bg-white p-6 rounded-2xl shadow-sm border border-orange-100 ring-1 ring-orange-50 flex items-center gap-4 hover:shadow-md hover:border-orange-200 transition group block">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-orange-600">{heroMetrics.pendingGrades}</p>
            </div>
          </Link>

          {/* Metric 3 */}
          <Link to="/notifications" className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 hover:shadow-md hover:border-slate-200 transition group block">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900">{heroMetrics.systemAlerts}</p>
            </div>
          </Link>
        </div>
      </section>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-5 rounded-2xl border border-blue-100/50 flex items-start gap-4 shadow-sm relative overflow-hidden">
          ...
          <div className="z-10 flex-1">
            ...
            <p className="text-sm text-slate-700 mt-1.5 leading-relaxed">
              Chào buổi sáng! Hôm nay bạn có <strong>3 lớp học</strong> (Toán 10A1, 10A2, 11B). Có <strong>15 bài tập</strong> môn Toán 10A1 đang chờ chấm. Học sinh <strong>Nguyễn Văn A</strong> (10A1) có dấu hiệu giảm sút điểm số ở 2 bài kiểm tra gần nhất, bạn nên lưu ý nhé!
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/classes" className="text-sm font-medium text-blue-600 hover:text-blue-700 transition flex items-center gap-1">
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">{cls.subject} • {cls.role}</p>
                    </div>
                  </div>
                  <div className="text-right flex items-center gap-3">
                    <div className="hidden sm:flex flex-col items-end">
                      <span className="text-sm font-medium text-slate-700 flex items-center gap-1">
                        <Users className="w-4 h-4 text-slate-400" /> {cls.studentsCount} học sinh
                      </span>
                    </div>
